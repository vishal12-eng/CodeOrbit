...
export class JavaRunner implements Runner {
  ...
  private compileJava(projectDir: string, entryFile: string): { success: boolean; output: string } {
    try {
      const output = execSync(`javac ${entryFile}`, {
        cwd: projectDir,
  ...
  async run(config: RunnerConfig, callbacks?: StreamCallbacks): Promise<RunResult> {
    ...
    const entryFile = config.entryFile || "Main.java";

    const compileResult = this.compileJava(config.projectDir, entryFile);
    ...
    return new Promise<RunResult>((resolve) => {
      ...
      try {
        this.process = spawn("java", [className], {
