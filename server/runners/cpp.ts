...
export class CppRunner implements Runner {
  ...
  private compileCpp(projectDir: string, entryFile: string): { success: boolean; output: string; outputBinary: string } {
    const outputBinary = path.join(projectDir, "app");
    try {
      const output = execSync(`g++ -o app ${entryFile}`, {
        cwd: projectDir,
  ...
  async run(config: RunnerConfig, callbacks?: StreamCallbacks): Promise<RunResult> {
    ...
    const entryFile = config.entryFile || "main.cpp";

    const compileResult = this.compileCpp(config.projectDir, entryFile);
...
export const cppRunner = new CppRunner();
