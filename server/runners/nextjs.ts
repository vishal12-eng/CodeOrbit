...
export class NextJsRunner implements Runner {
  ...
  private async buildProject(projectDir: string): Promise<{ success: boolean; output: string }> {
    const nextBuildPath = path.join(projectDir, ".next");

    if (existsSync(nextBuildPath)) {
      return { success: true, output: "Build already exists" };
    }
    ...
    try {
      ...
      return { success: true, output: output || "Build completed successfully" };
    } catch (error) {
      return {
        success: false,
  ...
  async run(config: RunnerConfig, callbacks?: StreamCallbacks): Promise<RunResult> {
    ...
    const buildResult = await this.buildProject(config.projectDir);
    if (!buildResult.success) {
      const result: RunResult = {
        success: false,
        stdout: installResult.output,
        stderr: buildResult.output,
        executionTime: Date.now() - startTime,
        exitCode: -1,
      };
      callbacks?.onComplete?.(result);
    ...
    callbacks?.onStdout?.("Building project...\n");

    return new Promise<RunResult>((resolve) => {
      let stdout = installResult.output + "\n" + buildResult.output + "\n";
      let stderr = "";
      let resolved = false;
      ...
      try {
        this.process = spawn("npm", ["start", "--", "-p", String(this.port)], {
...
export const nextJsRunner = new NextJsRunner();
